...
The Console runs R and is in the bottom left. The bottom right is the Output pane and gives a preview of the created html document as well as help and shows the associated files. The top left is the Source and it is for writing the code. The Environment pane is in the top right and shows what objects are part of the working space and its history.
...
`NA` means that there is no value imputed for that cell.
...
```{r plastic_waste_per_cap-hist-continent, eval=TRUE}
ggplot(data = plastic_waste, aes(x = plastic_waste_per_cap)) +
  geom_histogram(binwidth = 0.2) + facet_wrap(~continent) +
  labs(
    title = "Plastic Waste per Capita", subtitle = "Faceted by Continent",
    x = "Plastic Waste", y = "Count")
```
...
```{r plastic_waste_per_cap-dens-color-fill-alpha-adjusted, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     color = continent, 
                     fill = continent)) +
  ...
  labs(
    ...
    x = "Plastic Waste", y = "Count", color = "Continent", fill = "Continent")
```
...
`Color` and `fill` are set as mapping aesthetics because they are mapped to a variable. If a visual is supposed to change based on a variable it is mapped as an aesthetic. If a visual is a set value (like `alpha`), it is a characteristic of the plotting geom.
...
ggplot(data = plastic_waste, 
  ...
  labs(
    title = "Plastic Waste per Capita",
    x = "Continent", y = "Plastic Waste per Capita (daily kg/person)")
```
...
Unlike in the box plots, violin plots show the mode or where the most amount of values are. The distribution of countries is easier to visualize with the violin plots. The box plots, however, show the median and IQR of the data which is missed in the violin plots. They also better show the unusualness of some of the data points(such as Trinidad and Tobago). With an even larger data set, the violin plots could make it look like there were points in between.
...
```{r plastic_waste_and_missmanaged, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = mismanaged_plastic_waste_per_cap)) +
  geom_point(color = "blue", alpha = 0.20) +
  labs(
    title = "Plastic Waste and Mismanaged Plastic Waste per Capita",
    x = "Plastic Waste per Capita", y = "Mismanaged Plastic Waste per Capita")

plastic_waste %>%
  filter(mismanaged_plastic_waste_per_cap > 0.2)
```
    Describe the relationship.
    
There is a positive relationship between plastic waste per capita and mismanaged plastic waste per capita. As plastic waste increase, so does mismanaged plastic waste. However, it is not perfectly linear, and rather creates a fan shape as the values increase.

2.  Colour the points in the scatterplot by continent.

```{r plastic_waste_and_missmanaged-continent, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
  ...
  labs(
    title = "Plastic Waste and Mismanaged Plastic Waste per Capita",
    x = "Plastic Waste per Capita", y = "Mismanaged Plastic Waste per Capita", color = "Continent")
```
...
Yes, there are some trends in the association between plastic waste per capita and mismanaged plastic waste per capita. Africa and and Oceania have a very steep curve to the relationship, while Europe is much more mild. Asia, South America, and North America are somewhere in the middle of the two extremes.
...
```{r plastic_waste_total_pop-coastal_pop, eval=TRUE}
plastic_waste %>%
  #filter(plastic_waste_per_cap < 3) %>%
  ggplot(aes(x = total_pop, y = plastic_waste_per_cap)) +
  geom_point(color = "blue", alpha = 0.20) +
  labs(
    title = "Plastic Waste per Capita and Total Population",
    x = "Total Population", y = "Plastic Waste per Capita (daily kg/person")

plastic_waste %>%
  #filter(plastic_waste_per_cap < 3) %>%
  ggplot(aes(x = coastal_pop, y = plastic_waste_per_cap)) +
  geom_point(color = "green", alpha = 0.20) +
  labs(
    title = "Plastic Waste per Capita and Coastal Population",
    x = "Plastic Waste per Capita (daily kg/person", y = "Costal Population")

#Filtered out Trinidad and Tobago so that the points can be seen better.
...
If any, coastal population appears to have a slightly stronger linear association as it appears to more closely match a line. However, neither appear to truly be strongly linearly associated.
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    theme_minimal()
```
Plastic waster per capita sharp increases up to ~0.7 Coastal population proportion. The growth slows and then starts to decrease after 1. When less of the population is coastal it has a greater affect on the plastic waste per capita.
